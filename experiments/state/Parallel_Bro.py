"""
Parallel_Bro.py — Parallel lockstep state-based training with BroNet actor/critic.

Key properties of the current implementation:
  - state input, no separate encoder
  - parallel environment stepping via wrapper_parallel
  - BRCReplayBuffer + BRCRewardNormalizer parallel workflow
  - actor uses fresh task embeddings after critic updates
"""
...
from core.brc_components import (
    BRCTaskEmbedding, BRCTanhPolicy, BRCRewardNormalizer,
    TASK_EMB_DIM, ACTOR_WIDTH, ACTOR_DEPTH,
)
...
# =====================================================================
# UltimateBRCAgent — overrides FaithfulBRCAgent for full alignment
# =====================================================================

class UltimateBRCAgent(SACC51Mixin):
    """BRC agent with maximum fidelity to the original JAX implementation.

    Key differences from FaithfulBRCAgent:
      - weight_decay = 1e-4 (matching optax.adamw default)
      - update(): actor uses task_emb from UPDATED critic (not stale)
      - Critic target dist: mask is always 1.0 (BRC MetaWorld convention)
    """
    ...
    def __init__(self, device, num_tasks):
        ...
        actor_input_dim = STATE_DIM + TASK_EMB_DIM
        ...
        # --- Optimizers (weight_decay=1e-4, matching BRC optax.adamw) ---
    ...
    # ------------------------------------------------------------------
    # Critic update (mask always 1.0 — BRC MetaWorld convention)
    # ------------------------------------------------------------------

    def _update_critic(self, obs, action, reward, next_obs, task_emb):
        """C51 critic update. mask=1.0 always (BRC: MetaWorld terminal=False)."""
        metrics = {}
    ...
    # ------------------------------------------------------------------
    # Actor update (uses FRESH task_emb from updated critic)
    # ------------------------------------------------------------------

    def _update_actor(self, obs, task_emb_fresh):
        """Actor update using task_emb from the UPDATED critic.

        BRC original: update_actor receives new_critic, calls
        build_actor_input(new_critic, ...) to get fresh task_emb.
        """
        metrics = {}

        task_emb_d = task_emb_fresh.detach()
        obs_d = obs.detach()
    ...
    # ------------------------------------------------------------------
    # Full update (BRC order: critic -> soft_update -> actor -> alpha)
    # ------------------------------------------------------------------

    def update(self, batch):
        """BRC-faithful update order:
        1. Compute task_emb (pre-critic)
        2. Update critic (uses pre-critic task_emb)
        ...
        # Pre-critic task_emb (for critic update)
        task_emb = self.task_embedding(task_ids)

        # 1. Critic update
        critic_m = self._update_critic(obs, action, reward, next_obs, task_emb)

        # 2. Soft update target
        self._soft_update()

        # 3. Fresh task_emb from UPDATED critic/task_embedding
    ...
    def get_eval_actions(self, obs_np, task_ids_np, device):
        ...
        temb = self.task_embedding(tid_t)
        actor_in = torch.cat([obs_t, temb], dim=-1)
        dist = self.actor(actor_in, temperature=1.0)
...
def train(cfg: DictConfig):
    ...
    with read_write(cfg):
        ...
        cfg.env.action_repeat = 1       # BRC original: no action repeat
    ...
    # --- Agent (UltimateBRCAgent with full alignment) ---
    agent = UltimateBRCAgent(device, num_tasks)
    agent.train(True)
    ...
    if off_bs > 0:
        ...
        if offline_buffer.size == 0:
            print("  [WARN] Offline buffer is empty, disabling offline mixing.")
            offline_buffer = None
            off_bs = 0
            on_bs = BATCH_SIZE
    ...
    # --- Normalizer (always-bootstrap mode: no success distinction) ---
    ...
    # Warm up normalizer with offline data (always bootstrap, no success flag)
    if all_reward_seqs:
        print("Pre-updating Reward Normalizer with P + N datasets...")
        for gid, seqs in all_reward_seqs.items():
            lid = g2l[gid]
            for r_seq in seqs:
                # BRC original: always bootstrap (success=False → truncate path)
    ...
    # --- Parallel environment (unified 200 steps) ---
    # Override max_episode_steps to unified 200 for all tasks
    ...
    # --- Print config ---
    print(f"=== Parallel_Bro | {work_dir} ===")
    print(f"  state_dim={STATE_DIM} batch={BATCH_SIZE}(on={on_bs}+off={off_bs}) "
    ...
    print(f"  mask=always_1 (BRC convention) | actor_task_emb=fresh (post-critic)")
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        if step < WARMUP_PER_TASK:
            ...
            with torch.no_grad():
                ...
                temb = agent.task_embedding(tid_t)
                actor_in = torch.cat([obs_t, temb], dim=-1)
                dist = agent.actor(actor_in, temperature=1.0)
        ...
        if step >= WARMUP_PER_TASK and buffer.size >= BATCH_SIZE:
            ...
            for u in range(UTD):
                ...
                # terminated = 1 - mask (always 0 here, but keep for interface)
        ...
        if step % SAVE_INTERVAL == 0:
            ...
            torch.save(snap, work_dir / f'snapshot_parallel_bro_{step}.pt')
            torch.save(snap, work_dir / 'snapshot_latest.pt')
            print(f"  Saved snapshot at step {step}")

    logger.close()
    print("Parallel_Bro training finished.")
