"""
ON_TD3BC.py — RLPD-style Online Fine-tuning for TD3+BC (linear BC decay).

Uses 50% online + 50% offline (P+N) mixed batches.
Actor loss = bc_weight * BC_loss + lambda * Q_loss, bc_weight decays to 0.
No entropy regularisation (alpha = 0 throughout).
...
from experiments.pixel.offline.OFF_TD3BC import (
    TD3BCAgent, BC_ALPHA, POLICY_NOISE, NOISE_CLIP, POLICY_FREQ,
)
...
def _td3bc_format_log(m, global_step, buf_size):
    return (
        f"  [S={global_step:>7d}] "
        ...
        f"bc_w={m.get('bc_weight',0):.2f} "
        f"bc={m.get('bc_loss',0):.3f} "
        f"lm={m.get('lmbda',0):.2f} "
        f"buf={buf_size}")
...
def train(cfg: DictConfig):
    ...
    def _snapshot_extra(agent, payload):
        ...
        if '_critic_step_count' in payload:
            agent._critic_step_count = payload['_critic_step_count']

    agent, agent_ref = setup_dual_agents(
        cfg, device, TD3BCAgent,
        agent_kwargs=TD3BC_KWARGS, ref_kwargs=TD3BC_KWARGS,
        snapshot_extra_fn=_snapshot_extra)

    for p in agent_ref.actor_target.parameters():
    ...
    print(f"=== TD3+BC Online Fine-tuning | {Path.cwd()} ===")
    print(f"  batch={batch_size}(off={off_bs}[P={off_p}+N={off_n}]+on={on_bs}) UTD={UTD} "
          ...
          f"noise={POLICY_NOISE}(clip={NOISE_CLIP}) policy_freq={POLICY_FREQ}")
    print(f"  BC: alpha={BC_ALPHA} decay_steps={BC_DECAY_STEPS}")
    ...
    def update_fn(global_step):
        ...
        on_rews_n = normalizer.normalize(on_rews, g2l[on_gids], 0.0)
        ...
        if off_bs > 0:
            ...
            pix_m, act_m, rew_m, nxt_m, don_m, gid_m = mix_online_offline(
                ...
                bp, bn, off_p, off_n, normalizer, g2l, 0.0)
    ...
    run_online_training(
        ...
        checkpoint_prefix='snapshot_td3bc_post',
        update_fn=update_fn,
        format_log_fn=_td3bc_format_log,
        done_message="TD3+BC Online fine-tuning finished.")
